    ...
    methods: {
      ...
      getDialogTextForFirstTimeEverUser() {
        return [
          ...
          'Приложение AR-Nav пердназначено для облегчения ориентации в закрытых помещениях. ' +
          'Для этого мы используем технологию дополненной реальности.',

          'Навигация будет происходить пошагово. На каждом шаге Вам необходимо найти и сканировать AR-метку. ' +
          'После этого Вы получите указания, где найти следующую метку. Следуйте им, чтобы достигнуть своей цели.',

          'Указания предполагают, что Вы смотрите на метку под прямым углом. Оптимальная дистанция сканирования AR-метки - 30-40 см.',

          'Ну что ж, давайте начнем. Для начала вам необходимо выбрать цель. Цель - это объект, который вы хотите найти. ' +
          'Чтобы выбрать цель, найдите и сканируйте первую AR-метку.',
        ];
      },
      ...
      getDialogTextForReturnedUser() {
        return [
          'С возвращением в приложение AR-Nav! Давайте найдем AR-метку и выберем цель.',
        ];
      },
